...
RSpec.describe Resolvers::RequestSearch do
  describe "#resolve" do
    ...
    let!(:request1) { create(:request, user:, price: 19, created_at: Date.today) }
    let!(:request2) { create(:request, user:, price: 100, created_at: Date.today + 1) }
    let!(:request3) { create(:request, user:, price: 200, created_at: Date.today + 2) }
